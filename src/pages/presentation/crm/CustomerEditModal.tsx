...
interface CustomerEditModalInterface {
	id?: string | number
	isOpen?: boolean
	setIsOpen?: any
}

const CustomerEditModal = ({ id, isOpen, setIsOpen }: CustomerEditModalInterface) => {
	const itemData = id ? data.filter((item: any) => item.id.toString() === id.toString()) : [];
	const item: any = id ? itemData[0] : {};
