...
const EmployeeList = () => {
	...
	const searchUsers = Object.values(USERS)
		.filter(
			(user) =>
				user.username
					.toLowerCase()
					.includes(formik.values.searchInput.toLowerCase()) ||
				user.name.toLowerCase().includes(formik.values.searchInput.toLowerCase()) ||
				user.surname
					.toLowerCase()
					.includes(formik.values.searchInput.toLowerCase()) ||
				user.position.toLowerCase().includes(formik.values.searchInput.toLowerCase()),
		)
		.filter((user2) => (formik.values.available ? user2.isOnline : user2))
		.map((i) => i);
	return (
