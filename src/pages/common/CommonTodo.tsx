...
const CommonTodo = () => {
	...
	const getBadgeWithText = (text: string): any => {
		return Object.values(TODO_BADGES).filter((todoBadge) => todoBadge.text === text);
	};
	...
	const completeTaskLength = list.filter((i: any) => i.status).length;
	 ...
	 * @param badge
	 */
	const addTodo = (title: string, date: Date, badge: string) => {
								...
								<div className='col-12'>
									<FormGroup>
										...
										<ChecksGroup isInline>
											{Object.values(TODO_BADGES).map((i: any) => (
												<Checks
													key={i.text}
													type='radio'
													name='todoBadges'
