...
interface CommonRightPanelInterface {
	setOpen: (value: any) => void
	isOpen: boolean
}

const CommonRightPanel = ({ setOpen, isOpen }: CommonRightPanelInterface) => {
	...
	const handleActiveUserAppointmentTab = (tabName: string) => {
		...
		<OffCanvas setOpen={setOpen} isOpen={isOpen} isRightPanel>
			<OffCanvasBody className='p-4'>
				...
				<div className='d-flex flex-column align-items-center mb-5'>
					<div className='h2 fw-bold'>{`${USERS[0].name} ${USERS[0].surname}`}</div>
					<div className='h5 text-muted text-lowercase opacity-50'>{`@${USERS[0].name}${USERS[0].surname}`}</div>
				</div>
				<div
					...
					<div className='row row-cols-3 g-3 pb-3 px-3 mt-0'>
						{Object.values(USER_APPOINTMENT).map((value: string) => (
							<div
								key={value}
								...
								<Button
									color={
										(darkModeStatus &&
											activeUserAppointmentTab === value) ||
										!darkModeStatus
											? 'dark'
											: null
									}
									...
									rounded={1}
									onClick={() =>
										handleActiveUserAppointmentTab(value)
									}
									isLight={activeUserAppointmentTab !== value}>
