...
const BoardCard = ({ card, groupId, data, setData }: BoardCardInterface) => {
	useTourStep(13);
	...
	const formik = useFormik({
		initialValues: {
			...
			task:
				(card.tasks && card.tasks.filter((f: any) => f.status).map((m: any) => m.id.toString())) ||
				[],
			tags: (card.tags && card.tags.map((m: any) => m.id)) || [],
		},
		onSubmit: (values) => {
				...
				values.groupId === groupId
									...
									cards: gr.cards.map((crd: any) => {
										if (card.id === crd.id) {
											return {
												...
												tags: values.tags.map((m: any) => getTagsDataWithId(m)),
											};
										}
										return crd;
									}),
					...
					: prevData.map((gr: any) => {
							...
							if (gr.id === groupId) {
								return {
									...gr,
									cards: gr.cards.filter((crd: any) => crd.id !== card.id),
								};
							}
